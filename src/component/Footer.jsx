...
const SERVICES = [
  ...
  { name: 'Aluminum Siding', href: '#services' },
  { name: 'Aluminum Windows', href: '#services' },
  { name: 'Gutters & Downspouts', href: '#services' },
  { name: 'Roof Inspection', href: '#services' },
];

const QUICK_LINKS = [
  { name: 'About Us', href: '/about', isInternal: true },
  { name: 'Our Projects', href: '/projects', isInternal: true },
  { name: 'Contact Us', href: '/contact', isInternal: true },
  { name: 'Free Quote', href: '/contact', isInternal: true },
  { name: 'Warranty', href: '/warranty', isInternal: true },
  { name: 'FAQ', href: '/#faq', isInternal: false }, // Handled as standard anchor for hash compatibility
];

const SOCIAL_LINKS = [
  {
    name: 'Facebook',
    href: 'https://www.facebook.com/hoppyroofandaluminum',
    icon: (
  ...
  {
    name: 'Twitter',
    href: 'https://twitter.com/yaya_afeez',
    icon: (
      <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.395 0-.779-.023-1.158-.067a13.945 13.945 0 007.557 2.212c9.054 0 14-7.496 14-13.986 0-.21 0-.423-.015-.634A9.936 9.936 0 0024 4.59z"/>
      </svg>
    ),
  },
  {
    name: 'WhatsApp',
    href: 'https://wa.me/2349014592261',
    icon: (
      <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {SERVICES.map((service) => (
                <li key={service.name}>
                  <a href={service.href} className="text-slate-400 hover:text-amber-200 transition-colors text-sm">
                    {service.name}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              {QUICK_LINKS.map((link) => (
                <li key={link.name}>
                  {link.isInternal ? (
                    <Link to={link.href} className="text-slate-400 hover:text-amber-200 transition-colors text-sm">
                      {link.name}
                    </Link>
                  ) : (
                    <a href={link.href} className="text-slate-400 hover:text-amber-200 transition-colors text-sm">
                      {link.name}
                    </a>
                  )}
                </li>
        ...
        <div className="mt-8 pt-8 border-t border-white/10">
          <div className="flex flex-col sm:flex-row justify-between items-center">
            <p className="text-sm text-slate-400">
              &copy; {new Date().getFullYear()} Hoppy Roof & Aluminum. All rights reserved.
            </p>
            <div className="flex space-x-6 mt-4 sm:mt-0">
              <Link to="/privacy-policy" className="text-sm text-slate-400 hover:text-amber-200 transition-colors">Privacy Policy</Link>
              ...
              <button 
                ...
                className="text-sm text-slate-400 hover:text-amber-200 transition-colors cursor-pointer focus:outline-none"
              >
                Top
              </button>
            </div>       
